...
def preprocess_folder(data_path, recompute_bout_df=False,
                      recompute_filtering=False, recompute_regressors=False):
    ...
    for path in tqdm(paths):
        ...
        if not (path / "bouts_df.h5").exists() or recompute_bout_df:
            ...
            beh_df.loc[:, [f"theta_0{i}" for i in range(9)]] = \
                predictive_tail_fill(theta_mat)
            ...
            bouts_df["idx"] = [np.argmin((beh_df["t"] - bouts_df.loc[i, "t_start"]).abs())
        ...
        if not (path / "filtered_traces.h5").exists() or recompute_filtering:
            ...
            traces = preprocess_traces(traces_raw, fn, smooth_wnd_s=5.5,
                                       detrend_wnd_s=900)
            traces_und = preprocess_traces(traces_raw, fn, smooth_wnd_s=5.5,
                                           detrend_wnd_s=None)
        ...
        if not (path / "motor_regressors.h5").exists() or recompute_regressors:
            ...
            reg_dict = create_motor_regressors(traces.shape[0], bouts_df, fn, min_bias=0.05)
            ...
            reg_df = pd.DataFrame(np.concatenate([reg_mat, reg_mat_diff], axis=1),
                 columns=list(reg_dict.columns) + [c + "_dfdt" for c in reg_dict.columns])
...
if __name__=="__main__":
    ...
    preprocess_folder(Path("/Users/luigipetrucco/Desktop/source_data_batch1"),
                      recompute_bout_df=False, recompute_regressors=False)
    # preprocess_folder(Path("/Users/luigipetrucco/Desktop/source_data_batch1"),
