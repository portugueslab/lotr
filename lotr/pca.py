...
def get_fictive_trajectory(n_pts, bouts_df, min_bias=0.05):
    ...
    for i in bouts_df.index:
        if np.abs(bouts_df.loc[i, "bias"]) > min_bias:
            theta_turns[bouts_df.loc[i, "idx_imaging"]] = bouts_df.loc[i, "bias"] #* coef
...
def fictive_trajectory_and_fit(phase_unwrapped, bouts_df, fn=5, min_bias=0.05):
    ...
    fictive_trajectory = get_fictive_trajectory(len(phase_unwrapped), bouts_df, min_bias=min_bias)
